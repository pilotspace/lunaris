//! Plan 05-04 HELIOS-02 — dual-backend integration smoke for [`HeliosScratchpad`].
//!
...
//!    [`HeliosScratchpad::grep`]; asserts the recall p50 budget.
//!
//! Both `#[ignore]`-gated by default. Run via:
//!
//! ```bash
//! MOON_URL=moon://localhost:6390 PG_URL=postgres://lunaris@localhost/lunaris \
//!   cargo test -p lunaris --test helios_scratchpad_smoke -- --ignored --nocapture
//! ```
//!
...
async fn helios_chat_10k_turns_dual_backend() -> anyhow::Result<()> {
    for url_env in ["MOON_URL", "PG_URL"] {
        ...
        let pad = HeliosScratchpad::new(lunaris.clone(), lunaris_core::Scope::dev(), &session_id);
