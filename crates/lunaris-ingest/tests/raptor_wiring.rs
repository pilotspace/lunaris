...
use lunaris_core::{
    ...
    keyspace::{community_prefix, chunk_prefix},
...
async fn scan_deserialize<T: serde::de::DeserializeOwned>(
    ...
    let mut stream = storage
        ...
        .await
        .expect("scan_range must succeed");
...
async fn fixture_3a_h1_community_summary_non_empty() {
    let storage = EmbeddedStorage::connect("memory://")
        .await
        .expect("embedded storage must connect");
    ...
    let h1_communities: Vec<&Community> =
        communities.iter().filter(|c| c.level == 1).collect();
    assert!(
