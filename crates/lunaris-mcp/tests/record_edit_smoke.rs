//! Smoke test for `memory.record_edit`.
//!
...
//!   2. `tools/list` returns a tools array that includes `memory.record_edit`.
//!   3. `tools/call` for `memory.record_edit` returns a non-empty `lsn`.
...
//!
//! # RED phase
//! Before Task 2 registers `memory.record_edit`, the `tools/list` assertion
...
async fn record_edit_smoke() {
    ...
    let db_path = tmp.path().join("record_edit_smoke.db");
    ...
    // ── 2. tools/list — assert memory.record_edit is registered ──────────────
    ...
    assert!(
        tool_names.contains(&"memory.record_edit"),
        "memory.record_edit not in tools/list: {tool_names:?}"
    );

    // ── 3. tools/call memory.record_edit ─────────────────────────────────────
    let call_req = serde_json::json!({
        ...
        "params": {
            ...
            "arguments": {
                "path": "src/lib.rs",
                "after": "pub fn new() -> Self { Self }",
                "intent": "add constructor"
            }
        }
    });
    ...
    let output: serde_json::Value =
        serde_json::from_str(text).expect("parse record_edit response JSON");

    // Assert lsn is non-empty with wall_ms:counter format.
    let lsn = output["lsn"].as_str().expect("lsn field in record_edit response");
