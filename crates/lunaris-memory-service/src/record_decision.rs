...
pub async fn handle(
...
) -> Result<RecordDecisionResponse, ServiceError> {
    let source = format!("decision:{}", scope.as_str());
    ...
    let content = serde_json::to_string(&payload)
        .map_err(|e| ServiceError::InvalidInput(format!("serialize decision payload: {e}")))?;
