...
fn record_decision_handler_does_not_call_atomic_write_directly() {
    let path = concat!(env!("CARGO_MANIFEST_DIR"), "/src/record_decision.rs");
    let src = std::fs::read_to_string(path).expect("src/record_decision.rs must exist");
    ...
    assert!(
        ...
        "INGEST-04 violation: src/record_decision.rs must not call atomic_write directly. \
...
fn record_edit_handler_does_not_call_atomic_write_directly() {
    let path = concat!(env!("CARGO_MANIFEST_DIR"), "/src/record_edit.rs");
    let src = std::fs::read_to_string(path).expect("src/record_edit.rs must exist");
    ...
    assert!(
        !code_only.contains("atomic_write"),
        "INGEST-04 violation: src/record_edit.rs must not call atomic_write directly. \
         Use ScopedLunaris::ingest instead."
    );
}
