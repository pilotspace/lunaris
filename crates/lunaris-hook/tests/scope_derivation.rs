...
fn hook_scope_matches_core_scope_resolver() {
    ...
    let hook_scope = lunaris_hook::scope::resolve_with_path(td.path(), &scopes_path, None)
    ...
    let core_scope = resolve_with(td.path(), &mem_store, None).expect("core scope must resolve");
...
fn hook_scope_env_override_wins() {
    ...
    let scope = lunaris_hook::scope::resolve_with_path(td.path(), &scopes_path, Some("my-project"))
...
fn hook_scope_is_stable_across_calls() {
    ...
    let s1 =
        lunaris_hook::scope::resolve_with_path(td.path(), &scopes_path, None).expect("first call");
    let s2 =
        lunaris_hook::scope::resolve_with_path(td.path(), &scopes_path, None).expect("second call");
