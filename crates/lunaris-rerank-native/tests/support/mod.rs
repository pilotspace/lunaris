...
//! `lunaris.rerank.*` instrumentation (see
...
//!
//! Mirrors `lunaris-embed-native/tests/support/mod.rs` byte-for-byte (same
//! capture strategy on both sides of the embed/rerank split). Not a
//! `tests/*.rs` file itself (no `#[test]`s here) — `tests/support/mod.rs` is
//! a module included via `mod support;`, NOT compiled as its own harness
