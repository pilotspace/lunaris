...
# Also build the bundled Moon server (mq + graph + text-index default).
...
Use `memory.status` from MCP to verify Moon capabilities and MQ-backed
`__lunaris_verify__` / `__lunaris_consolidate__` queue health.
