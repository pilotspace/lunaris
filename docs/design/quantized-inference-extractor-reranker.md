...
| `bge-reranker-v2-m3` (reranker) | Cross-encoder rerank stage — **opt-in, default OFF** (`LUNARIS_RECALL_RERANK`; GA-1 audit 2026-08-17 found no production surface invoked it before then, despite this table's original "every production recall" claim) | FP32 `NativeReranker` **default**; Q5_K_M-imatrix GGUF exists but is **opt-in** (`LUNARIS_RERANKER_GGUF` + `reranker-gguf` feature). Quantized path already routes through `device_select` → Metal on Apple Silicon. | FP32 = 2.3 GB weights + FP32 gemm. The Q5 GGUF (447 MB) is what all recent LME validation runs actually use — the shipped default is the path nobody runs. |
