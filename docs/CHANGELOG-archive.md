# Changelog archive — pre-0.6.0 raw accumulation

> The project's `CHANGELOG.md` was never cut into versioned sections before
> the 0.6.0-rc.1 release (git tags v0.1.0–v0.5.0 exist as tags only). This file
> preserves the full raw `## Unreleased` accumulation verbatim as it stood at
> the 0.6.0-rc.1 cut, so no history is lost. The public log restarts, curated,
> at `## [0.6.0-rc.1]` in CHANGELOG.md.
