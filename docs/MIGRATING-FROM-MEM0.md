# Migrating from Mem0 to Lunaris

Lunaris and Mem0 occupy adjacent niches in the agent-memory space.
This doc maps Mem0 concepts to their Lunaris equivalents so a team
...
and the type system rejects mixing incompatible operators.
...
hits = await (
    mem.scoped(scope)
       .recall()
       .vector("chunks", 10)
       .as_of(snapshot_ts)         # ← bi-temporal cut
       .execute(lunaris.Query.text("what did Alice do at Acme?"))
)
...
away the evidence that it ever existed.
