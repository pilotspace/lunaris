...
| **Bi-temporal at the storage layer**    | `.as_of(ts)` pushes the temporal cut into the query, not a Python post-filter — on the **search and graph** lanes. A historical **KV** read is refused on Moon (`read_as_of` past a 1-hour live window returns `NotSupported` → HTTP 501). | `crates/lunaris/src/recall.rs`; refusal pinned by `crates/lunaris-conformance/tests/run_as_of_moon_gap.rs` |
...
- **Ecosystem (shipped)** — LangGraph / CrewAI / Letta adapters via the
  ...
  it from a checkout, see [`integrations/README.md`](../integrations/README.md)). The "drop Lunaris into your existing agent framework"
