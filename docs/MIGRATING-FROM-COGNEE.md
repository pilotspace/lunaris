# Migrating from Cognee to Lunaris
...
This doc maps Cognee concepts to their Lunaris equivalents so a team
...
# Graph extraction is opt-in via .with_graph_pipeline(True).
```
...
hits = await (
    mem.scoped(scope)
       ...
       .fuse_rrf(60)
       .top(5)
       .execute(lunaris.Query.text("who does Alice work with at Acme?"))
)
```
...
typed expression; no LLM round-trip for the fusion step.
...
v0.3 RFC 0007 (`FallbackExtractor` / `FallbackEmbedder` combinators)
adds the fan-out primitive.
...
2. **Map `dataset` → `Scope`.** Same `[A-Za-z0-9_\-.]{1,128}`
   alphabet constraint as the Zep migration.
...
See `docs/MIGRATING-FROM-MEM0.md` and `docs/MIGRATING-FROM-ZEP.md`
for the parallel migration stories from the other two incumbents.
The trio covers the three distinct positioning conversations:
Mem0 (no bi-temporal upgrade required), Zep (latency + substrate
simplification), Cognee (pipeline-vs-DSL tradeoff).
