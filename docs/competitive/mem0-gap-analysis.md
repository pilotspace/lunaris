...
| 4 | correctness / security | ahead | one `atomic_write`, JWT-bound scope, Postgres RLS USING+WITH CHECK on every read path; Mem0 has app-layer-only isolation + a graph-delete data-retention bug |
...
| 8 | SDK / DX / integrations | partial(built-not-wired) | Python/TS SDKs + MCP (11 tools) ship, but LangGraph/CrewAI/Letta adapters are promised in docs yet absent in code |
...
| correctness-security | app-layer user_id scoping; no RLS; graph delete leaves orphans (#3245) | one atomic_write; JWT-bound scope; RLS USING+WITH CHECK + set_config every read | crates/lunaris-ingest/src/pipeline.rs:assemble_and_write | https://github.com/mem0ai/mem0/issues/3245 (2026-06-14) | ahead | P2 |
...
| sdk-dx | Python/TS/Go SDKs + 13 framework integrations + hosted API | Python/TS SDKs + MCP (11 tools); LangGraph/CrewAI/Letta adapters promised but absent | crates/lunaris-py/src/lib.rs | https://docs.mem0.ai/integrations (2026-06-14) | partial(built-not-wired) | P1 |
...
(`npm i @pilotspace/lunaris`) ship full surfaces; `lunaris-mcp` ships 11 tools as a universal shim. But
