...
- [ ] extractor-fallback-wiring       P0  depends-on: io-failsafe-wiring  — **deferred Half A of the split**: wire the already-built `FallbackExtractor`+`CircuitBreaker` onto the production `default_extractor()`/`open()` path (today both are test-only — built ≠ wired). Transient extractor failure degrades to NoopExtractor, terminal propagates.
- [ ] observability-rollout-maturity P1  depends-on: none  — `/healthz` storage probe (returns 503 on dead Moon) + wire `lunaris_eval_score` (high ROI, S effort)
