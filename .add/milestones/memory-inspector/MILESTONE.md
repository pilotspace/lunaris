...
- [x] **HUMAN-UAT** (2026-06-16): Tin ran a live browser UAT against a Moon-backed server — it did its job, surfacing the graph surface bug (`root`+`depth` ignored; "show all nodes on empty id"), root-caused to Moon silently dropping the inline-property anchor and fixed to the WHERE-form in PR #37 (+ a live-Moon regression test). Operator then accepted the test-covered contract (48/48 + the new live-Moon graph test) in lieu of a post-fix browser re-pass ("skip the relaunch / close it out"), so the four surfaces are gate-accepted rather than re-UAT'd green. Recorded as the operator's human-gate decision, not an automated pass.
