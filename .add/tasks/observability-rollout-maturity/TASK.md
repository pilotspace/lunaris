...
- [TDD · open] STRUCTURAL guard (`include_str!` source assertion in a separate file) + BEHAVIORAL seam test is the reusable pattern when the real backend path needs un-CI-able infra — here a live Moon's full connect dialog; the bounding guarantee piggybacks on the existing `op_timeout` test (evidence: `health_check_ping.rs` green; reused from extractor-fallback-wiring).
- [ADD · open] `cargo fmt` over §4 test files DURING build trips the scope-lock test-tamper flag; the clean clear is `add.py phase tests`→re-advance to re-snapshot at the formatted state (evidence: this task AND io-failsafe both hit it). Suggests an fmt-aware scope-walk or a "fmt-only test diff ≠ tampering" carve-out.
- [SDD · open] additive-default trait method (`Ok(())` default + ONE backend override) keeps blast radius to one trait + one backend — every other `StoragePort` impl compiled unchanged (evidence: `cargo check --workspace` green with only Moon overriding `health_check`). The `queue_depth`/`hot_keys` precedent generalizes to operator-global probes.
