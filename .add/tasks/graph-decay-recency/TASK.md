...
- [TDD · folded] "additive trait method + capability flag" is now a twice-proven recipe (queue_depth, graph_traverse_decayed) — adding a field to the literal-constructed `StorageCapabilities` costs one mechanical perl pass over ~40 sites and the compiler catches every miss; consider `#[non_exhaustive]` + builder only if literal churn becomes a real tax (evidence: clean first compile after the sweep).
- [SDD · folded] mirroring the upstream fixture (Moon's DECAY-01/02 in scripts/test-commands.sh) resolved the freeze's lowest-confidence ⚠ before build — WEIGHT is an ADDEDGE argument, not a Cypher property (evidence: §3 flag closed in §4 fixture note; flip test green first run).
- [DDD · folded] production GraphEdge writes carry no explicit WEIGHT, so decay on real Lunaris graphs is age-only re-ranking today (evidence: atomic.rs GraphEdge writer; §7 spec delta feeds ft-navigate-recall).
- [ADD · folded] stale sibling services on default ports turn graceful-skip tests into hard failures (tree_recall on old Moon @ 6380: connect succeeds, FT._LIST missing) — graceful-skip should distinguish "unreachable" from "reachable but incompatible" (evidence: clean-tree stash repro).
