...
- [SDD · open] probe-before-freeze paid for itself twice: the ADDNODE prop-coercion hazard (digit-hex → Float) and the GRAPH.SETPROP docs-drift (documented, unimplemented) were both caught by live probes BEFORE the contract was built against them (evidence: /tmp probes 2026-06-11; §5 build notes).
