# TASK: Root-cause the ~0.8s CPU cost of every lunaris.open() call and restore a fast open path

slug: open-call-latency · created: 2026-06-11 · stage: production
...
Feature: open-call-latency (SPLIT from ts-specs-v04-refresh, 2026-06-11)

Standing evidence (measured, /tmp/cb-venv editable build, warm process, XDG_CACHE_HOME at an EMPTY dir):
  - `await lunaris.open("memory://")`                      -> 834ms / 889ms per call
  - `await lunaris.open("unsupported-scheme://nowhere")`   -> 800ms / 773ms per call (raises STORAGE)
  - wall 807ms vs process-CPU 797ms for one call -> pure CPU, not sleep/backoff/network
  - cost is PER-CALL (not per-process init) and identical for success and parse-failure
  - consequence: test_gil_discipline's "sub-ms error path" premise died ->
    "50 concurrent GIL-releasing awaits took 8.8186s > 1.0s" (GIL itself is fine, ~4x overlap)
Suspects to investigate first: per-open construction of a heavy component on the
