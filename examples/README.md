...
|---|---|---|---|
| Quickstart — Rust | [quickstart-rs/](quickstart-rs/) | open → ingest → scoped `recall()` (one-liner + typed DSL) | Moon (`--shards 1`); `cargo add lunaris-memory --rename lunaris` |
| Quickstart — Python | [quickstart-py/](quickstart-py/) | ingest contract (recall is v0.3 for the binding) | Moon (`--shards 1`); `pip install lunaris` |
| Quickstart — TypeScript | [quickstart-ts/](quickstart-ts/) | ingest contract (recall is v0.3 for the binding) | Moon (`--shards 1`); `npm i lunaris` |
...
The three quickstarts reuse the **same** docker-compose Moon
(`quickstart-rs/docker-compose.yml`) so a developer can stand up one container
and exercise all three SDKs against it. 0.7.0 is Moon-only — the shared
`pg-lunaris` Postgres image the quickstarts used through 0.6.x was deleted with
the backend (see [`docs/migration/0.6-to-0.7.md`](../docs/migration/0.6-to-0.7.md)).
`multi-agent-rs` builds its handle by hand (`Lunaris::with_parts_keyword` +
`StubEmbedder`) so it needs no external services beyond that same single-shard
Moon — see its README for the runbook.
